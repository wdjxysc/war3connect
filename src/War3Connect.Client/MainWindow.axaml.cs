...
{
    private sealed record GamePreferences(string Path, string? Wine = null, string? WinePrefix = null);
    private sealed record Settings(string Server, string Username, string Game, string? Wine = null, string? WinePrefix = null, bool AllowInsecureHttp = false,
        string SelectedGame = GameCatalog.War3, Dictionary<string, GamePreferences>? Games = null);
    ...
    public MainWindow(bool loadSettings)
    {
        ...
        try
        {
            if (loadSettings && File.Exists(_settingsPath) && JsonSerializer.Deserialize<Settings>(File.ReadAllText(_settingsPath)) is { } s)
            {
                ...
                _gamePreferences = s.Games ?? new();
                _gamePreferences.TryAdd(GameCatalog.War3, new(s.Game, s.Wine, s.WinePrefix));
                _selectedGame = GameCatalog.IsKnown(s.SelectedGame) ? s.SelectedGame : GameCatalog.War3;
        ...
        GameSelector.SelectedIndex = _selectedGame == GameCatalog.StarCraft ? 1 : 0;
        LoadGamePreferences();
        _gameUiReady = true;
    ...
    private async Task Login(bool register)
    {
        ...
        StatusText.Text = IsStarCraft ? StarCraftNotice : "选择游戏后，可以创建或加入平台房间；地图在 War3 内选择。";
        SaveSettings();
        await RefreshRooms();
    }
    ...
    private async Task RefreshRooms()
    {
        ...
        try
        {
            ...
            var visible = rooms.Where(r => r.GameId == _selectedGame).ToArray();
            RoomsList.ItemsSource = visible;
            RoomsList.SelectedItem = visible.FirstOrDefault(r => r.Id == selectedId);
        }
    ...
    {
        var files = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions { Title = $"选择 {GameCatalog.Executable(_selectedGame)}", FileTypeFilter = [new("游戏程序") { Patterns = patterns }] });
        if (files.Count > 0) { target.Text = files[0].TryGetLocalPath(); Inspect(); }
    });
    private GameInstallation Inspect()
    {
        ...
        var install = GameInstallation.Inspect(GamePathBox.Text, _selectedGame);
        VersionText.Text = $"版本 {install.Version}\n" + (IsStarCraft ? "已识别程序；联机转发尚未接入。" : "地图选择、下载和校验由 War3 处理。");
        SaveSettings();
        return install;
    }
    private async void LaunchClick(object sender, RoutedEventArgs e) => await Run(() =>
    {
        ...
        StatusText.Text = IsStarCraft ? "已启动星际。平台尚未提供跨网联机，请使用游戏自身的局域网功能。" : "已启动游戏，请进入局域网；游戏端口使用 6112。";
        return Task.CompletedTask;
    });
    private async void CreateClick(object sender, RoutedEventArgs e) => await Run(async () =>
    {
        RequireLogin();
        var install = Inspect();
        var room = await _api!.Post<RoomView>("api/rooms", new CreateRoom((RoomNameBox.Text ?? "").Trim(), (RoomPasswordBox.Text ?? ""), install.Version, GameId: _selectedGame));
        await Enter(room);
    });
    private async void JoinClick(object sender, RoutedEventArgs e) => await Run(async () =>
    {
        ...
        var install = Inspect();
        var room = await _api!.Post<RoomView>($"api/rooms/{selected.Id}/join", new JoinRoom((RoomPasswordBox.Text ?? ""), install.Version, _selectedGame));
        await Enter(room);
    });
    ...
    private async Task Enter(RoomView room)
    {
        ...
        AppendLog(IsStarCraft ? StarCraftNotice : room.HostId == _api!.Session!.UserId ? "已创建房间，等待游戏内建图。" : "已加入房间，等待房主游戏公告。");
        StatusText.Text = IsStarCraft ? StarCraftNotice : room.HostId == _api!.Session!.UserId ? "房间已创建。请启动 War3，在局域网中选择地图建图。" : "已加入房间。请启动 War3，在局域网列表中等待并加入房主游戏。";
        await RefreshRooms();
    }
    private void ShowRoom(RoomView room)
    {
        ...
        GameStatusText.Text = room.GameId == GameCatalog.StarCraft ? StarCraftNotice : room.Game == null
    ...
    private async Task ClearRoom()
    {
        ...
        GameStatusText.Text = IsStarCraft ? StarCraftNotice : "房主建图后，玩家进入 War3 → 局域网加入。";
        UpdateButtons();
    }
    ...
    {
        if (!_persistSettings) { RememberGamePreferences(); return true; }
        try
        {
            ...
            var legacy = _gamePreferences.GetValueOrDefault(GameCatalog.War3) ?? new GamePreferences("");
            var content = JsonSerializer.Serialize(new Settings(address, UsernameBox.Text ?? "", legacy.Path, legacy.Wine, legacy.WinePrefix,
                AllowHttpBox.IsChecked == true, _selectedGame, _gamePreferences));
